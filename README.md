...
GitHub Actions also runs a Release build on every push to `main` or `master` and uploads a zipped `MDViewer.app` workflow artifact. Pushing a version tag such as `v0.1.11` also creates a GitHub Release with `MDViewer.app.zip` attached.
